...
export function FollowingProjectsTab({ userId }: FollowingProjectsTabProps) {
  ...
  const { data: followedProjects = [], isLoading } = useQuery<ProjectWithRelations[]>({
    ...
    queryFn: async () => {
      ...
      return data.map((item: { projects: ProjectWithRelations }) => item.projects);
    },
    enabled: !!userId,
  });
  ...
  const unfollowProject = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['followed-projects-details'] });
      toast.success('Proyecto eliminado de guardados');
    },
    onError: () => {
      toast.error('Error al dejar de seguir el proyecto');
    }
  });
    ...
    <div className="w-full">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {followedProjects.map((project) => {
          ...
          return (
            <Card key={project.id} className="overflow-hidden group hover:shadow-md transition-shadow h-full flex flex-col">
              <Link to={`/projects/${project.id}`} className="h-full flex flex-col">
                ...
                <div className="relative aspect-video bg-muted overflow-hidden">
                  ...
                  <Badge 
                    className={`absolute top-2 left-2 ${getEngineColor(project.engine)} text-primary-foreground border-0 text-[10px]`}
                  >
                    {engineLabel}
                  </Badge>
                </div>
